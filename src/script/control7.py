...
def process_credit_card(path):
    ...
    # Lógica de Categorização com Fallback (Se não achar no JSON, usa o do CSV)
    def enrich(row):
        desc = str(row['description'])
        if desc in lookup:
            return lookup[desc][0], lookup[desc][1]
        # Fallback: pega a categoria do CSV se existir, senão 'Outros'
        cat_original = row.get('raw_category', 'Outros')
        return cat_original, "Aguardando Classificação"

    df[['category', 'subcategory']] = df.apply(lambda x: pd.Series(enrich(x)), axis=1)
    ...
    valid_cols = ['transaction_date', 'description', 'category', 'subcategory', 'amount_brl', 'card_holder_name', 'source_file', 'transaction_type']
    return df[[c for c in valid_cols if c in df.columns]]

def ingest_historical_file(file_name):
    """Segmenta a rodada para um arquivo específico na pasta xlsx e salva no lake."""
    file_path = HISTORICAL_XLSX_DIR / file_name
    print(f"⏳ Processando arquivo histórico: {file_name}...")
    
    df_cc = process_credit_card(str(file_path))
    
    if not df_cc.empty:
        # --- NOVO: Lógica para salvar no Data Lake ---
        # Extrai o ano do nome do arquivo (ex: de '2022-09.xlsx' pega '2022')
        year = file_name.split('-')[0]
        target_folder = LAKE_DIR / year
        target_folder.mkdir(parents=True, exist_ok=True)
        
        # Define o nome do novo CSV (ex: invoice-2022-09.csv)
        csv_name = f"invoice-{file_name.replace('.xlsx', '.csv').replace('.csv', '')}.csv"
        final_csv_path = target_folder / csv_name
        
        # Salva a cópia processada no Data Lake
        df_cc.to_csv(final_csv_path, index=False, sep=';', encoding='utf-8')
        print(f"📂 Cópia salva em: {final_csv_path}")
        # --------------------------------------------
        ...
        df_cc.to_sql('fact_transactions', engine, if_exists='append', index=False)
        print(f"✅ Sucesso! {len(df_cc)} linhas inseridas para {file_name}.")
...
if __name__ == "__main__":
    ...
    run_pipeline(mode="historical", target_file="2023-01.xlsx")