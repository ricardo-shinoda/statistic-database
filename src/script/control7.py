...
#     if not CONTROLE_XLSX.exists(): return pd.DataFrame()
#     try:
#         # Se você tirou as linhas de título, lemos sem o skiprows
#         df = pd.read_excel(CONTROLE_XLSX, sheet_name='Consumo Nissan Kicks')
        
#         # Limpa nomes das colunas (remove espaços invisíveis)
#         df.columns = [str(c).strip() for c in df.columns]

#         # LISTA RÍGIDA: Apenas o que existe na tabela 'fact_vehicle_fueling' do Postgres
#         db_columns = [
#             'filling_date', 'odometer_reading', 'total_amount_paid', 'discount_amount',
#             'volume_liters', 'fuel_type', 'price_per_liter', 'kmv_points', 
#             'cashback_amount', 'gas_station', 'comp_fuel_type', 'comp_price_per_liter',
#             'board_consumption_kml', 'board_avg_speed_kmh', 'board_travel_time',
#             'board_remaining_range_km'
#         ]
        
#         # Filtra o DataFrame para conter APENAS essas colunas, ignorando "Valor Descontado" e outros
#         valid_cols = [c for c in db_columns if c in df.columns]
#         df = df[valid_cols].copy()

#         # Garante que temos a coluna de data
#         if 'filling_date' not in df.columns:
#             print(f"❌ Coluna 'filling_date' não encontrada. Colunas atuais: {df.columns.tolist()}")
#             return pd.DataFrame()

#         df = df.dropna(subset=['filling_date'])
        
#         # Limpeza numérica para evitar o erro de Bilhões (overflow)
#         numeric_cols = [
#             'odometer_reading', 'total_amount_paid', 'discount_amount', 'volume_liters',
#             'price_per_liter', 'kmv_points', 'cashback_amount', 'comp_price_per_liter',
#             'board_consumption_kml', 'board_avg_speed_kmh', 'board_remaining_range_km'
#         ]
        
#         for col in numeric_cols:
#             if col in df.columns:
#                 df[col] = pd.to_numeric(clean_currency(df[col]), errors='coerce').fillna(0)

#         if 'board_travel_time' in df.columns:
#             df['board_travel_time'] = df['board_travel_time'].astype(str)
        
#         return df
#     except Exception as e:
#         print(f"⚠️ Erro no Nissan: {e}")
#         return pd.DataFrame()
